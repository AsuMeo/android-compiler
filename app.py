...
<resources>
    <string name="app_name">{app_name}</string>
        ...
        # Компилируем каждый XML ресурс отдельно
        compiled_files = []
        for res_file in res_files_list:
            if res_file.endswith(".xml"):
                flat_name = os.path.basename(res_file).replace(".xml", ".flat")
                # Для values используем простое имя
                if "/values/" in res_file:
                    flat_name = os.path.basename(res_file).replace(".xml", ".arsc.flat")
                flat_path = os.path.join(compiled_res_dir, flat_name)
                r = subprocess.run([aapt2, "compile", "--legacy", "-o", compiled_res_dir, res_file],
                    capture_output=True, text=True)
                if r.returncode != 0:
                    log(f"[3/9] ⚠ {os.path.basename(res_file)}: {r.stderr[:200]}")
                else:
                    # aapt2 может создать файл с другим именем
                    for f in os.listdir(compiled_res_dir):
                        if f.endswith(".flat") and f not in compiled_files:
        ...
        log("[4/9] ✓ Ресурсы слинкованы, R.java создан")
