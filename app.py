...
<resources>
    <string name="app_name">{app_name}</string>
        ...
        # ПРАВИЛЬНО: используем --dir вместо compiled_res.zip
        link_args = [
            aapt2, "link",
            ...
            "--version-name", "1.0",
            "--auto-add-overlay",
            res_dir  # <-- ПРАВИЛЬНО: папка ресурсов напрямую
        ]

        r = subprocess.run(link_args, capture_output=True, text=True)
        if r.returncode != 0:
            log(f"[4/8] ✗ ОШИБКА aapt2 link:")
        ...
        if r.returncode != 0:
            log(f"[6/8] ✗ ОШИБКА d8: {r.stderr[:300]}")
            return {"success": False, "error": r.stderr[:500], "logs": logs}
...
<title>⚡ Java → APK Compiler 2026</title>
<style>
:root { --bg:#0a0a0f; --bg2:#12121a; --bg3:#1a1a2e; --border:#2a2a3e; --text:#e0e0ff; --text2:#8b8bb5; --accent:#7c3aed; --accent2:#a855f7; --accent3:#c084fc; --success:#22c55e; --error:#ef4444; --warn:#f59e0b; }
...
.logs { background:var(--bg); border:1px solid var(--border); border-radius:12px; padding:14px; font-family:'SF Mono',monospace; font-size:11px; color:var(--text2); max-height:200px; overflow-y:auto; margin-top:14px; white-space:pre-wrap; word-break:break-all; display:none; }
...
async function buildApk() {
  ...
  try {
    setStatus('⏳ Отправка на сервер...', 'info');
    ...
    if (result.success && result.downloadUrl) {
      ...
      setStatus('❌ Ошибка: ' + (result.error || 'Неизвестная ошибка'), 'error');
    }
  } catch(e) {
    setStatus('❌ Ошибка сети. Сервер спит? Подожди 30 сек и попробуй снова.<br><small>' + e.message + '</small>', 'error');
  }
